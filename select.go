...
func (s *Select) reRender(config *PromptConfig) {
	options := s.filterOptions(config)
	pageSize := s.PageSize
	if pageSize == 0 {
		pageSize = config.PageSize
...
func (s *Select) Prompt(config *PromptConfig) (interface{}, error) {
	...
	ch := make(chan runeRet)
	ack := make(chan struct{})
	quit := make(chan struct{})
	go func(ch chan runeRet, ack chan struct{}, quit chan struct{}) {
		for {
			select {
			case <-quit:
				return
	...
	for inputFlag {
		...
		case <-time.After(time.Millisecond * time.Duration(config.UpdateInterval)):
			s.reRender(config)
		}
	}
	options := s.filterOptions(config)
